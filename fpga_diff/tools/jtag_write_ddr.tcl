...
proc write_to_ddr {fn} { 
  ...
  while {[eof $fdata] != 1} {
    ...
    # Get the length of DataString
    set len [expr {[string length $DataString] / 8}]
    # Check if len is reasonable
    set max_len 256;
    if {$len > $max_len} {
    ...
    # Use len for AXI write operation
    ...
    # Calculate the length of the completed progress bar
    set completed_length [expr {int(($percent * $progress_bar_length) / 100)}]
    # Build the '#' and space part of the progress bar
