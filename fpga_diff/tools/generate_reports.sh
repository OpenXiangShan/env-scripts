...
if [ -z "${BASH_VERSION:-}" ]; then
  echo "ERROR: this script requires bash. Run with: bash $0 <CPU>  or: CPU=<name> $0" 1>&2
  exit 1
fi
...
if [ -n "$1" ]; then
  ...
  echo "Usage: $(basename "$0") <CPU>  OR  CPU=<name> $(basename "$0")" 1>&2
...
if [ ! -f "$TCL_SCRIPT" ]; then
  echo "ERROR: tcl script not found: $TCL_SCRIPT" 1>&2
  exit 1
fi

# Check that the Vivado project file exists
if [ ! -f "$PROJ_FILE" ]; then
  echo "ERROR: project file not found: $PROJ_FILE" 1>&2
  exit 1
fi
...
```# filepath: /nfs/home/fengkehan/env-scripts/fpga_diff/tools/generate_reports.sh
# ...existing code...
#!/usr/bin/env bash
# Require bash (avoid running with `sh` which can cause "bad substitution")
if [ -z "${BASH_VERSION:-}" ]; then
