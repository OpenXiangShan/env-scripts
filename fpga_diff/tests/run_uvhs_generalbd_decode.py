...
--functional-only checks GBS1 FIFO/drain behavior. Omitting it also runs
the strict hole sweep, which reports its known disagreement with the core's
reserved decoded-zero range. The test rejects the old raw-address decode
mutation. --skip-mutation runs only the baseline.
...
def extract(source):
    ...
    block = resolve_conditionals(source[first:last])
...
def main():
    ...
    block = extract(original.decode())
    top = "uvhs_generalbd_decode_tb"
    ...
    with tempfile.TemporaryDirectory(prefix="uvhs_generalbd_decode_") as tmp:
        for name, code in variants.items():
            ...
            command = ["/usr/local/bin/verilator", "--binary", "--timing", "--assert", "-Wall", "-Wno-fatal",
                       ...
                       f"-I{work}", str(rtl / "uvhs_gbus_c2h_fifo.sv"),
