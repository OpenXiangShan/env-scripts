...
// The existing Makefile CPU selection emits CPU_NUTSHELL for NutShell.  Keep
// the added memory-array interface aligned with the CPU RTL DDR AXI port:
// NutShell is 64 bits, while XiangShan-family targets are 256 bits.
`ifndef UVHS_CPU_DDR_AXI_DATA_WIDTH
`ifdef CPU_NUTSHELL
`define UVHS_CPU_DDR_AXI_DATA_WIDTH 64
`else
`define UVHS_CPU_DDR_AXI_DATA_WIDTH 256
`endif
`endif
...
module uvhs_axi_to_mem_array #(
  ...
  parameter int AXI_DATA_WIDTH = `UVHS_CPU_DDR_AXI_DATA_WIDTH
) (
  input  logic                         clk,
  input  logic                         rstn,
...
module uvhs_mem_array_ddr_wrapper #(
  ...
  localparam logic [9:0] UVW_AXI_DATA_WIDTH = AXI_DATA_WIDTH[9:0];
  ...
  uvw_mem_array_wrapper #(
    .BRING_UP(0),
    .UVW_PORT_NUM(4),
    .UVW_USE_DATA_WIDTH({4{UVW_AXI_DATA_WIDTH}}),
    .UVW_DATA_WIDTH({4{UVW_AXI_DATA_WIDTH}}),
