...
// XiangShan releases that enable the DiffTest clock gate provide the
// implementation in generated-src/rtl/DifftestClockGate.v.  Import the
// generated macro header before declaring compatibility cells so the UVHS
// stub does not define a second module with the same name.
...
`ifndef CONFIG_DIFFTEST_CLOCKGATE
...
module AXI_bridge (
  ...
  SYS_INTER_CLK, SYS_INTER_ARESETN, UART_0_rxd, UART_0_txd, UART_CLK,
...
// AXI_bridge's Vivado DCP contains a dedicated UART clock pin.  Keep it in
// the synthesis stub so the DCP clock is explicitly driven by the SoC clock
// at the integration site rather than becoming a driverless net in PnR.
input UART_CLK;
