...
#
# The defaults mirror the validated Hejian XDMA EP x4 configuration:
...
export UVHS_XDMA_EP_DCP_SHA256="${UVHS_XDMA_EP_DCP_SHA256:-e40002b4ccab73d4d7a1398630f59428add0a64b9f33d284d3789595bb791b32}"
export UVHS_XDMA_EP_EXPECTED_VIVADO="${UVHS_XDMA_EP_EXPECTED_VIVADO:-2024.2}"
export UVHS_BASE_ASSEMBLE_FILE="${UVHS_BASE_ASSEMBLE_FILE:-}"
export UVHS_BASE_ASSEMBLE_SHA256="${UVHS_BASE_ASSEMBLE_SHA256:-0e44f3ced8e5628a6f9b60a36d15194f03f6eddefcf29d0221a8abf140ecb18b}"
...
uvhs_hejian_pcie_check_env() {
  ...
  if [[ -n "$UVHS_XDMA_EP_DCP" ]]; then
    ...
    if [[ -n "$dcp_vivado" ]]; then
      ...
      if [[ "$dcp_vivado" != "$UVHS_XDMA_EP_EXPECTED_VIVADO" ]]; then
        printf 'FAIL: XDMA DCP Vivado version=%s, expected %s\n' "$dcp_vivado" "$UVHS_XDMA_EP_EXPECTED_VIVADO" >&2
        failures=$((failures + 1))
      fi
    ...
    printf 'FAIL: UVHS_XDMA_EP_DCP is unset; point to the verified XDMA x4 xdma_ep.dcp\n' >&2
    failures=$((failures + 1))
  fi
  if [[ -n "$UVHS_BASE_ASSEMBLE_FILE" ]]; then
    ...
    printf 'FAIL: UVHS_BASE_ASSEMBLE_FILE is unset; point to the verified x4 1B_4F_HGC_assemble.tcl\n' >&2
    failures=$((failures + 1))
  fi
...
Preferred fpga_diff build entry:
  ...
  make -C env-scripts/fpga_diff uvhs_hejian_pcie_x4_xiangshan_all CORE_DIR=/path/to/XiangShan-release/build
