...
# SOC_GATED_CLK is generated by DifftestClockGate from CPU_CLK_IN/sys_clk_i.
# Keep it in the CPU timing group so CPU <-> gated-SoC paths remain timed
# instead of being hidden behind async false paths.
...
set fpga_diff_xdma_axi_clocks [fpga_diff_collect_clocks {
    M00_AXIS_ACLK*
    XDMA_AXI_ACLK
...
# XDMA AXI clock follows XDMA_LINK_WIDTH (X4=125 MHz, X8=250 MHz).
# The UVHS black-box boundary exposes xdma_0/axi_aclk as TO_DIFFTEST_PCIE_CLK,
# so classify that inferred clock as XDMA AXI instead of a separate 100 MHz
# DiffTest PCIe domain.
set fpga_diff_xdma_axis_clocks $fpga_diff_xdma_axi_clocks
...
fpga_diff_set_async_pair xdma_axis_clock_converter \
    $fpga_diff_difftest_pcie_clocks \
    $fpga_diff_xdma_axis_clocks
...
unset -nocomplain fpga_diff_xdma_axis_clocks
