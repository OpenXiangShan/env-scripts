...
class GCPT(object):
    ...
    def get_trace_path(self):
        ...
        trace_path = os.path.join(
            self.trace_dir, self.benchspec, f"{self.benchspec}_{str(self.point)}.trace"
        )
                ...
                for line in f:
                    if "ABORT at pc" in line or "FATAL:" in line or "Error:" in line:
                        ...
                        if "cycleCnt = " in line:
                            ...
                            self.total_num_cycles = int(
                                cycle_cnt_str.replace(",", "").strip()
                            )
                        if "instrCnt = " in line:
                            instr_cnt_str = line.split("instrCnt =")[1].split(", ")[0]
                            self.total_num_instrs = int(
                                instr_cnt_str.replace(",", "").strip()
                            )
                        if "Host time spent" in line:
                            second_cnt_str = line.split("Host time spent:")[1].replace(
                                "ms", ""
                            )
                            self.num_seconds = (
                                int(second_cnt_str.replace(",", "").strip()) / 1000
                            )
            ...
            with open(self.get_out_path()) as f:
                for line in f:
                    if "dump wave to" in line:
                        wave_path = (
                            line.replace("...", "").replace("dump wave to", "").strip()
                        )
    ...
    def show(self):
        ...
        attributes_str = ", ".join(
            map(lambda k: f"{k:>8} = {str(attributes[k]):>9}", attributes)
        )
