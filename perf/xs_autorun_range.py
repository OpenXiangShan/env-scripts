...
class GCPT(object):
  ...
  def __init__(self, base_path, benchspec, point, weight, insts):
...
def load_all_gcpt(gcpt_path, json_path, state_filter=None, xs_path=None, sorted_by=None):
  ...
  for benchspec in data:
    ...
    for point in data[benchspec]["points"]:
      ...
      total_insts = data[benchspec]["insts"]
      gcpt = GCPT(gcpt_path, benchspec, point, weight, total_insts)
...
def xs_run(workloads, xs_path, warmup, max_instr, threads):
  emu_path = os.path.join(xs_path, "emu")
...
def xs_report_ipc(xs_path, gcpt_queue, result_queue):
  while not gcpt_queue.empty():
    ...
    if counters["IPC"] is not None:
      print(gcpt.total_insts)
      result_queue.put([gcpt.benchspec, [float(gcpt.weight), float(counters["IPC"]), float(gcpt.total_insts)]])
    else:
...
def xs_report(all_gcpt, xs_path, spec_version, isa, num_jobs):
  ...
  xs_report_ipc(xs_path, gcpt_queue, result_queue)
  ...
  for benchspec in gcpt_ipc:
    ...
    num_instr = list(map(lambda info: info[2], gcpt_ipc[benchspec]))
    print(f"{benchspec:>25} coverage: {total_weight:.2f}")
    num_seconds = total_cpi * num_instr[0] / (frequency * (10 ** 9))
...
if __name__ == "__main__":
  ...
  parser.add_argument('--begin', '-b', default=0, type=int, help="Range begin")
  parser.add_argument('--end', '-e', type=int, help="Range end")
  ...
  if args.end is None:
    gcpt = gcpt[args.begin:]
  else:
    gcpt = gcpt[args.begin:args.end]
